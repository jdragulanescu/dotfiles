...
<objective>
Create all phases necessary to close gaps identified by `/sky:audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/sky:add-phase` per gap.
</objective>

<execution_context>
<!-- Spawns sky-planner agent which has all planning expertise baked in -->
</execution_context>
...
```
No audit gaps found. Run `/sky:audit-milestone` first.
```
...
**Also available:**
- `/sky:execute-phase {N}` — if plans already exist
...
`/sky:audit-milestone` — re-audit to verify gaps closed
`/sky:complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/sky:plan-phase` next
</success_criteria>
