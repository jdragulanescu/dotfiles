...
Output: {phase}-UAT.md tracking all test results. If issues found: diagnosed gaps, verified fix plans ready for /sky:execute-phase
</objective>

<execution_context>
@/Users/skylight/.claude/sky/workflows/verify-work.md
@/Users/skylight/.claude/sky/templates/UAT.md
</execution_context>
...
8. If issues found:
   ...
   - Spawn sky-planner in --gaps mode to create fix plans
   - Spawn sky-plan-checker to verify fix plans
   - Iterate planner ↔ checker until plans pass (max 3)
   - Present ready status with `/clear` then `/sky:execute-phase`
</process>
...
- /sky:execute-phase {Z+1} — skip to execution (if already planned)
...
**Also available:**
- /sky:complete-milestone — skip audit, archive directly
...
- /sky:plan-phase {Z} --gaps — regenerate fix plans
...
**Options:**
- /sky:plan-phase {Z} --gaps — retry fix planning with guidance
- /sky:discuss-phase {Z} — gather more context before replanning
...
- [ ] If issues: sky-planner creates fix plans from diagnosed gaps
- [ ] If issues: sky-plan-checker verifies fix plans (max 3 iterations)
- [ ] Ready for `/sky:execute-phase` when complete
</success_criteria>
